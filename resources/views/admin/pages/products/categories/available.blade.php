...
@section('title', "Categorias para o Produto: {$product->title}")
...
@section('content_header')
        ...
        <div class="card-header">
            <nav aria-label="LaraFood BreadCrumb">
                <ol class="breadcrumb text-dark">
                    ...
                    <li class="breadcrumb-item">
                        <a href="{{ route('product.categories', $product->id) }}">Categorias</a>
                    </li>
    ...
    <h1 class="mb-2">Categorias disponíveis para: <b>{{ $product->title }}</b></h1>
...
@section('content')
    <div class="card">
        <div class="card-header">
            <form action="{{ route('product.categories.available', $product->id) }}" method="POST" class="form">
                @csrf
                <div class="row">
                    ...
                    <form action="{{ route('product.categories.attach', $product->id) }}" method="POST">
                        ...
                        @foreach ($categories as $category)
                            <tr>
                                ...
                                <td>
                                    <label class="font-weight-normal" role="button" for="category{{ $category->id }}">
                                        {{ $category->name }}
                                    </label>
                                </td>
                            </tr>
